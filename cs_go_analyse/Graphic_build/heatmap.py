...
def plot_map_list_of_game(
    ...
    try:
        ...
        loop_len = nb_games
    if premade != []:
        dataframe_position = dataframe_position[
            dataframe_position["name"].isin(premade)
        ].reset_index()
        color_set = "Match_ID"
        loop_len = nb_games
    if not dataframe_position.empty:
            ...
            ax.scatter(
                [
                    dataframe_position["x"][
                        (
                            (dataframe_position[color_set] == i)
                            & (dataframe_position["Bombsite"] == "A")
                        )
                    ]
                ],
                [
        ...
        if text:
            texts = [
                plt.text(
                    dataframe_position["x"][i],
                    dataframe_position["y"][i],
                    str(dataframe_position["info"][i]),
                    fontsize=7,
                    color="white",
                )
...
def plot_from_simple_df(dataframe_position, map, title='Plot position'):
    ...
    texts = [
        plt.text(
            dataframe_position["x"][i],
            dataframe_position["y"][i],
            str(dataframe_position["player"][i])
            + ","
            + str(dataframe_position["clock"][i])
            + ","
            + str(dataframe_position["weapon"][i]),
            fontsize=7,
            color="white",
        )
        for i in range(len(dataframe_position))
